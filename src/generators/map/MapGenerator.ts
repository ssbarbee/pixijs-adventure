...
export class MapGenerator {
  ...
  createEmptyMap(width: number, height: number): TileType[][] {
    return Array.from({ length: height }, () => Array.from({ length: width }, () => 'wall0'));
  }
  ...
  createRoom(isCentral: boolean): Room {
    ...
    const roomX = isCentral
      ? this.mapWidth / 2 - roomWidth / 2
      : Math.floor(Math.random() * (this.mapWidth - roomWidth));
    const roomY = isCentral
      ? this.mapHeight / 2 - roomHeight / 2
      : Math.floor(Math.random() * (this.mapHeight - roomHeight));
  ...
  connectRooms(): void {
    ...
    this.rooms.slice(1).forEach((room) => {
      ...
      for (
        let x = Math.min(roomCenter.x, centralPoint.x);
        x <= Math.max(roomCenter.x, centralPoint.x);
        x++
      ) {
      ...
      for (
        let y = Math.min(roomCenter.y, centralPoint.y);
        y <= Math.max(roomCenter.y, centralPoint.y);
        y++
      ) {
