...
export function generateWorld(width: number, height: number): { world: string[][]; playerStartingX: number; playerStartingY: number } {
    const map = initializeMap(width, height);

    const world: string[][] = Array.from({ length: width }).map((_, x) =>
        Array.from({ length: height }).map((__, y) =>
            map._map[x][y] === 1 ? getRandomString([WALL0, WALL1]) : getRandomString([GRASS0, GRASS1, GRASS2])
        )
    );
